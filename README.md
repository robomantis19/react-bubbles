...
Tokens are used for login authentication, they are consumed by cryptographic functions, the input that is and a token is output.
...
It stores the code for different websites accross the world.
It actually runs the code that hosts the website to let it run code.
